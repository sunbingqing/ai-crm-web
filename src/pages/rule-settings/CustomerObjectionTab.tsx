...
import { getObjectionOptions, saveObjectionOptions } from '@/services/rule-settings'
import type { ObjectionOption } from '@/services/rule-settings'

const NAME_MAX = 12
const DESC_MAX = 200

export function CustomerObjectionTab() {
  const [options, setOptions] = useState<ObjectionOption[] | null>(null)
  ...
  const { data: serverOptions, isLoading } = useQuery({
    queryKey: ['objectionOptions'],
    queryFn: getObjectionOptions,
    staleTime: Infinity,
  })
  ...
  const { mutate, isPending: saving } = useMutation({
    mutationFn: saveObjectionOptions,
    onSuccess: () => {
      toast.success('保存成功')
      pendingRef.current = true
      queryClient.invalidateQueries({ queryKey: ['objectionOptions'] }).then(() => {
  ...
  const nameError = useCallback((option: ObjectionOption) => {
    return errors[String(option.id)]?.name
  }, [errors])

  const descError = useCallback((option: ObjectionOption) => {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl">
      <div className="mb-6 flex items-start justify-between">
        <div>
          <h2 className="text-lg font-semibold">客户异议</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            配置成交阻碍类型，帮助识别客户犹豫、拒绝或推进卡点
          </p>
        </div>
      ...
      {mergedOptions.filter((o) => o.enabled).length === 0 && Object.keys(errors).length === 0 && (
        <p className="mb-4 text-xs text-destructive">请至少启用一个异议选项</p>
      )}

      <div className="mb-6 rounded-lg border bg-card p-4">
        <h3 className="text-sm font-semibold">异议选项</h3>
